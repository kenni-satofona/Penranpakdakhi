...
if ($status == 'tidak aktif') {
    ...
    // More headers
    $headers .= 'From: Dompet-Qu <noreply@penrandakhi.com>'."\r\n" . 'Reply-To : '.$name.' <'.$email.'>'."\r\n";
...
elseif($status == 'aktif') {
    $name = 'Admin Penran-Dakhi';
    $subject = 'Pengaktifan Akun Penran-Dakhi';
    $message = '<p style="text-align: center">Selamat suadara/i <b>'.$username.'!</b>, 
                ...
                <h3 style="text-align: center"><b>atas waktunya, kami ucapkan terimakasih!</b></h3>
                <a style="text-align: center" href="http://penrandakhi.000webhostapp.com" target="_blank">Gunakan Penran-Dakhi</a>';
    ...
    // More headers
    $headers .= 'From: Penran-Dakhi <noreply@penrandakhi.com>'."\r\n" . 'Reply-To : '.$name.' <'.$email.'>'."\r\n";
    $headers .= 'Cc: admin@yourdomain.com' . "\r\n"; //untuk cc lebih dari satu tinggal kasih koma
    @mail($to,$subject,$message,$headers);
}
?>